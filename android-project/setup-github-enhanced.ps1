# ============================================
# 儿童产品设计助手 - 自动化部署脚本（增强版 - Windows PowerShell）
# 版本: 1.1.0
...
function Convert-ToBase64 {
    ...
    # 提示如何复制
    Print-Info "💡 提示: 使用记事本打开 keystore_base64.txt，全选复制内容"
}
...
function Setup-RemoteRepo {
    ...
    Print-Info "检测到预配置的仓库信息："
    Print-Info "  GitHub 用户名: $($PRECONFIGURED_USERNAME)"
    Print-Info "  仓库名称: $($PRECONFIGURED_REPO_NAME)"
    Print-Info "  完整地址: https://github.com/$PRECONFIGURED_USERNAME/$PRECONFIGURED_REPO_NAME"
    Write-Host ""
    
    $response = Read-Host "是否使用预配置的仓库？(Y/n)"
    
    if ($response -ne 'n' -and $response -ne 'N') {
        $GITHUB_USERNAME = $PRECONFIGURED_USERNAME
        $REPO_NAME = $PRECONFIGURED_REPO_NAME
        Print-Success "使用预配置仓库"
    } else {
    ...
    Print-Warning "⚠ 请确保您已在GitHub上创建了仓库: $REPO_NAME"
    Print-Info "仓库创建地址: https://github.com/new"
    $response = Read-Host "仓库已创建吗？(y/N)"
    
    if ($response -ne 'y' -and $response -ne 'Y') {
        Print-Info "请先在GitHub上创建仓库，然后重新运行此脚本"
        Print-Info "仓库名称建议: $REPO_NAME"
        exit 1
    }
...
function Push-ToGitHub {
    ...
    Print-Info "正在推送代码..."
    Print-Warning "⚠ 可能需要输入GitHub凭据"
    Print-Info "如果使用密码认证失败，请使用Personal Access Token"
    Print-Info "获取Token: https://github.com/settings/tokens"
    Write-Host ""
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Print-Info ""
        Print-Info "建议使用Personal Access Token进行认证："
        Print-Info "1. 访问: https://github.com/settings/tokens"
        Print-Info "2. 点击 'Generate new token (classic)'"
        Print-Info "3. 选择权限: repo, workflow"
        Print-Info "4. 生成并复制token"
        Print-Info "5. 推送时，在密码提示处粘贴token"
        exit 1
    }
}
...
function Show-SecretsGuide {
    ...
    Write-Host ""
    Write-Host "1️⃣  访问 Secrets 配置页面："
    Write-Host "   https://github.com/$GITHUB_USERNAME/$REPO_NAME/settings/secrets/actions"
    Write-Host ""
    Write-Host "2️⃣  点击 'New repository secret'"
    Write-Host ""
    Write-Host "3️⃣  添加以下4个密钥："
    Write-Host ""
...
function Show-NextSteps {
    ...
    Print-Info "📚 接下来的操作:"
    Write-Host ""
    Write-Host "   1️⃣  ✅ 代码已推送到GitHub"
    Write-Host "   2️⃣  📝 配置GitHub Secrets（见上方指南）"
    Write-Host "   3️⃣  ⚙️ GitHub Actions会自动开始构建APK"
    Write-Host "   4️⃣  📥 在Actions页面下载构建好的APK"
    Write-Host ""
    Print-Info "🔗 快速链接:"
    Write-Host "   • 仓库地址: https://github.com/$GITHUB_USERNAME/$REPO_NAME"
    Write-Host "   • Actions监控: https://github.com/$GITHUB_USERNAME/$REPO_NAME/actions"
    Write-Host "   • Secrets配置: https://github.com/$GITHUB_USERNAME/$REPO_NAME/settings/secrets/actions"
    Write-Host ""
    Print-Info "🛠️ 本地构建测试:"
...
# 主函数
function Main {
    Print-Header "儿童产品设计助手 - 自动化部署（增强版）"
    ...
    Write-Host ""
    Print-Info "📦 预配置仓库: $PRECONFIGURED_USERNAME/$PRECONFIGURED_REPO_NAME"
    Write-Host ""
